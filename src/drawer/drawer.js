...
export function createDrawer(drawerType) {
  function Drawer({ $canvas, isWorker }) {
    this.$canvas = $canvas;
    this.contextGL = null;
    ...
    this.isWorkerRender = typeof OffscreenCanvas !== "undefined";
    this.isWorker = isWorker;

    if (this.isWorkerRender && !this.isWorker) {

      // 支持离屏渲染，创建一个worker渲染
      const BackgroundDrawer = Util.workerify(createDrawer, [
        "drawNextOutputPicture",
      ]);
      this.bgDrawer = new BackgroundDrawer({ isWorker: true });
      this.renderer = this.$canvas.getContext("bitmaprenderer");
    } else if (this.$canvas) {
      this.init();
    }
  }
  ...
  Drawer.prototype = {
    ...
    drawNextOutputPicture({ width, height, data }) {
      if (!this.isWorker && this.isWorkerRender) {

        // 离屏渲染模式，将数据发送给worker线程
        return this.bgDrawer
          .drawNextOutputPicture({ width, height, data })
          .then((data) => {
            this.renderer.transferFromImageBitmap(data);
          });
      } else if (!this.$canvas && this.isWorker) {
  ...
  // 支持离屏渲染时，收听线程信息
  if (typeof drawerType === "undefined") {
    let drawer = null;
    self.onmessage = ({ data: { id, type, data } }) => {
      switch (type) {
        case "constructor":
          drawer = new Drawer(data);
          break;
        case "drawNextOutputPicture":
          if (drawer) {
            const bitmap = drawer.drawNextOutputPicture(data);
            self.postMessage({ id: id, data: bitmap }, [bitmap]);
          }
          break;
      }
    };
